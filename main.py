...
if config.FRONT:
    ...
    btn_scale_plus = front.Button(
        "+", (nav_w * .735, nav_h * .66), (btn_w // 3.2, btn_h / 1.7), btn_f)
    btn_speed_plus = front.Button(
        "+", (nav_w * .735, nav_h * .6), (btn_w // 3.2, btn_h / 1.7), btn_f)

    btn_scale_minus = front.Button(
        "-", (nav_w * .15, nav_h * .66), (btn_w // 3.2, btn_h / 1.7), btn_f)
    btn_speed_minus = front.Button(
        "-", (nav_w * .15, nav_h * .6), (btn_w // 3.2, btn_h / 1.7), btn_f)

    btn_start = front.Button("START", (nav_w * .05, nav_h * .5),
                             (nav_w * .4525, nav_h * .09), btn_f, bg="green4", feedback="PAUSE")
    btn_step = front.Button(
        "STEP", (nav_w * .5225, nav_h * .5), (nav_w * .4525, nav_h * .09), btn_f)

    btn_add_cells = front.Button(
        "ADD", (nav_w * .74, nav_h * .4), (btn_w // 1.8, btn_h), btn_f)
    btn_del_cells = front.Button(
        "DEL", (nav_w * .05, nav_h * .4), (btn_w // 1.8, btn_h), btn_f)
    btn_reset = front.Button(
        "RESET", (nav_w * .3, nav_h * .4), (btn_w, btn_h), btn_f)

    btn_help = front.Button("HELP", (nav_w * .05, nav_h * .2),
                            (nav_w * .4525, nav_h * .09), btn_f)
    btn_fullscreen = front.Button(
        "SCREEN", (nav_w * .5225, nav_h * .2), (nav_w * .4525, nav_h * .09), btn_f)

    btn_exit = front.Button(
        "EXIT", (nav_w * .5225, nav_h * .1), (nav_w * .4525, nav_h * .09), btn_f)
    btn_options = front.Button("OPTIONS", (nav_w * .05, nav_h * .1),
                               (nav_w * .4525, nav_h * .09), btn_f, feedback="<- BACK")
...
def NAVBAR_surfaces_main():
    surfaces = [
        ...
        [btn_add_cells.surface, btn_add_cells.x,
            config.NAVBAR_HEIGHT - btn_add_cells.y],
        [btn_del_cells.surface, btn_del_cells.x,
            config.NAVBAR_HEIGHT - btn_del_cells.y],
        [btn_scale_plus.surface, btn_scale_plus.x,
            config.NAVBAR_HEIGHT - btn_scale_plus.y],
        [btn_scale_minus.surface, btn_scale_minus.x,
            config.NAVBAR_HEIGHT - btn_scale_minus.y],
        [btn_speed_plus.surface, btn_speed_plus.x,
            config.NAVBAR_HEIGHT - btn_speed_plus.y],
        [btn_speed_minus.surface, btn_speed_minus.x,
            config.NAVBAR_HEIGHT - btn_speed_minus.y],
        ...
        [btn_fullscreen.surface, btn_fullscreen.x,
            config.NAVBAR_HEIGHT - btn_fullscreen.y],
...
def main_cycle():
    ...
    # random, 0  rectangle, 1  circle, 2
    cell_style = cell_styles.index("rectangle")
            ...
            if result:
                ...
                if result[0] == 'FULLSCREEN':
                    ...
                    if btn_start.rect.collidepoint(result[1]) or 'SPACE' in result:
                        PLAY = not PLAY
                        if PLAY:
                            btn_start.change_text(
                                btn_start.feedback, bg="darkorange3")
                        else:
                        ...
                        btn_fullscreen.change_text(
                            btn_fullscreen.feedback, bg="red")

                    elif btn_options.rect.collidepoint(result[1]):
                        options = not options
                        if options:
                            btn_options.change_text(
                                btn_options.feedback, bg="grey40")
                        else:
                            btn_options.change_text(
                                btn_options.feedback, bg="black")
